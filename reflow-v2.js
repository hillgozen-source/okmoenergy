(function () {
  ...
  function applyLayout(value) {
    ...
    CONFIG.layout = {
      imageFit: /^(fill|cover|contain)$/.test(String(row.imageFit)) ? String(row.imageFit) : "fill",
      ...
      shopY: clampNumber(row.shopY, 8, 94, 84),
      returnX: clampNumber(row.returnX, 12, 88, 50),
      returnY: clampNumber(row.returnY, 8, 97, 94)
    };
  }
  ...
  function applyRemoteConfig(row) {
    ...
    var layout = {};
    try { layout = typeof row.layout_json === "string" ? JSON.parse(row.layout_json) : row.layout_json; } catch (_) {}
    applyLayout(layout);
  }
  ...
  function showRecommendation() {
    ...
    if (CONFIG.mediaType === "video" && CONFIG.mediaUrls[0]) {
      ...
      mediaHtml = '<div class="rf-media rf-image"><img class="rf-main-media" src="' + escapeHtml(CONFIG.mediaUrls[0]) + '" alt="Featured OKMO product recommendation"></div>';
    }
    ...
    overlay.innerHTML =
      ...
      '#okmoenergy-reflow-overlay .rf-media img,#okmoenergy-reflow-overlay .rf-media video{display:block;width:100%;height:100%;object-fit:var(--rf-fit);object-position:var(--rf-media-x) var(--rf-media-y);transform:scale(var(--rf-scale));transform-origin:var(--rf-media-x) var(--rf-media-y)}' +
      '#okmoenergy-reflow-overlay .rf-main-media{position:absolute;z-index:1;inset:0}' +
